...
export function RegistreSortant(): React.JSX.Element {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const acteur = useActeur();
  const parametres = useParametres();
  const courriers = useLiveQuery(() => db.courriers.where('sens').equals('SORTANT').reverse().sortBy('creeLe')) ?? [];
  ...
  async function exporterCsv() {
    ...
    lien.href = url;
    lien.download = `registre-sortant-${new Date().toISOString().slice(0, 10)}.csv`;
    lien.click();
  ...
  async function exporterRegistrePdf() {
    ...
    const blob = await genererRegistrePdf({
      parametres,
      titre: 'Registre chronologique — départ',
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-100">{t('nav.registreSortant')}</h1>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variante="primaire" onClick={() => navigate('/courriers/sortants/nouveau')}>
